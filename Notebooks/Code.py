...
#drop null data in "Distrubtion Pattern" column
filtered_df = fda_1.dropna(subset=['Distribution Pattern'])

#loop through each row in the fda_1 DataFrame with group "Year", "Month", and "Reason Category"
for (year, month, reason), group_df in filtered_df.groupby(['Year', 'Month', 'Reason Category']):
    state_counts = defaultdict(int)

    for i, row in group_df.iterrows():
        pattern = row["Distribution Pattern"].lower()
        nationwide = str(row.get("Nationwide", "")).strip().lower()
...
state_counts_df = pd.DataFrame(results)

#extract the dataframe into csv file
state_counts_df.to_csv("state_counts.csv", index=False)


#extrac the fda_1 dataframe into csv file
fda_1.to_csv("clean_fda_data_rev1.csv")
